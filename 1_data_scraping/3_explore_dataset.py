...
def create_data_report(dated_df):
    ...
    dated_df['month'] = [d.to_pydatetime().month
                             for d in dated_df.date]
    fig2 = dated_df.loc[dated_df.year.isin(range(2020,2021))].year.plot.hist().get_figure()
    ...
    for side in ['pro','anti']:
        df = pd.DataFrame(columns=['outlet']+list(range(2020,2022)))
        for outlet in top_domains[side]:
            row = [outlet]
            counts = dated_df.loc[dated_df.pretty_domain == outlet].year.value_counts()
            row.extend([counts[int(c)] if c in counts else 0 for c in range(2020,2022)])
    ...
    ax1.set_xticklabels(rotation=0,labels=range(2020,2022))
    ax2.set_xticklabels(rotation=0,labels=range(2020,2022))
    n = 4
    for ax in [ax1,ax2]:
...
if __name__ == "__main__":
    ...
    arg_parser.add_argument('--input_data_filename', type=str, default='output/filtered_dedup_combined_df_2020_1_1_to_2021_5_1.pkl', help='/path/to/dataset/to/explore')
